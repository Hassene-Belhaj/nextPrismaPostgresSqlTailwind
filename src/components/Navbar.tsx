...
const Navbar = ({ user }: IuserProps) => {
  ...
  return (
    <nav className={`${navScroll ? "w-full fixed z-50 bg-slate-100 shadow-lg" : "w-full fixed z-50  bg-white"}`}>
      ...
      <div className="md:hidden h-20 w-full bg-white relative">
        <button onClick={() => setToggleMenu(!toggleMenu)} className="absolute top-[50%] translate-y-[-50%] right-4 z-50">
          {toggleMenu ? <AiOutlineClose size={30} color="gray" /> : <FiMenu size={30} />}
        </button>
      </div>
      <div className={toggleMenu ? "xs:flex md:hidden absolute flex-col justify-center items-center top-0 left-0 w-full h-screen bg-black/90 ease-in duration-300" : "absolute top-0 left-[-100%] w-full h-screen bg-black/90  ease-out duration-300"}>
